...
date: 2020-06-26T10:24:00-05:00
---
...
As Windows Terminal is ```open-source```, people can do anything they want with the source to customize it. And due to this open-source nature, Windows Terminal became popular amongst all kinds of users and even developers.
...
First, we need to open up the menu and then press on ```Settings```. Alternatively, you can use ```Ctrl+,``` as well for the same.
...
As you click on the ```Settings``` option, you should see a notepad window show up. This is the JSON file which is basically the configuration file for Windows Terminal. I would recommend you to install Visual Studio Code as it makes editing the JSON much easier.
...
Assuming you’ve already installed Visual Studio Code and have opened the JSON file, let’s start customizing!
...
There are 9 including themes. We can set different themes for different profiles. Here, I’ve customized my WSL profile to use the ```Solarized Dark``` theme. To make changes to your JSON, locate the profile section in the ```settings.json``` and add ```”colorScheme”``` property to the profile you want to theme. The resultant modifications would look something like:
...
### Fonts
Windows Terminal supports any monospaced fonts. We can also set a default font size. You can also change the font size on the fly by using ```Ctrl + your mouse scroll wheel``` or ```Ctrl + ’+’ and ‘-‘```. 
...
### Adding icons for your terminal ![linux](https://raw.githubusercontent.com/sourajitk/sourajitk.github.io/source/data/windows%20terminal%20assests/ubuntu.png?token=AKZAGQINPGQMLTYX37W5FJS7AHVBU)  ![statix](https://raw.githubusercontent.com/sourajitk/sourajitk.github.io/source/data/windows%20terminal%20assests/statix.png?token=AKZAGQMLXFRG6G72SMTYEV27AHVAM)
First, we need to navigate to this directory ```%LOCALAPPDATA%\Packages\Microsoft.WindowsTerminal_8wekyb3d8bbwe\RoamingState```.

After you paste this in your Explorer address bar, you should now be in an empty folder.
